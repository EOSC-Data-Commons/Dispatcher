...
def vre_from_zipfile(self, parsed_zipfile: tuple[Dict, bytes], token):
    ...
    vre_handler = vre_factory(
        crate=crate, body=zip_file, token=token, update_state=self.update_state,task=self.request.id
    )
...
def vre_from_rocrate(self, data: Dict, token):
    ...
    vre_handler = vre_factory(crate=crate, token=token, update_state=self.update_state,task=self.request.id)
    return {"url": vre_handler.post()}
